...
export default function TestimonialsSection() {
  ...
  return (
    <section id="testimonials" className="py-24 bg-[#090D16] relative border-t border-white/10">
      <div className="container max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonialsData.map((item) => (
            <div
              key={item.id}
              className="glass-card p-7 flex flex-col justify-between border border-white/10 relative group bg-slate-950/70"
            >
              <div>
                {/* Author row */}
                <div className="flex items-center gap-3.5 mb-4">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden bg-slate-800 border border-white/20 shrink-0 shadow-md">
                    <Image
                      src={item.avatar}
                      alt={item.author}
                      fill
              ...
              <div>
                <div className="text-xs text-cyan-400 font-semibold bg-cyan-950/60 border border-cyan-500/30 px-3 py-1 rounded-full mb-4 inline-block">
                  Dự án: {item.project}
                </div>
